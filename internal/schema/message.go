...
const (
	RoleSystem    Role = "system"    // 系统提示词：确定agent的性格与红线
	RoleUser      Role = "user"      // 用户输入 / 工具的返回结果
	RoleAssistant Role = "assistant" // 模型的输出：包含推理或者工具调用
)
...
type Message struct {
	...
	// 如果这是对某个工具调用的响应，此字段必须填写，以告知模型上下文的关联性 ToolCallID string `json:"tool_call_id,omitempty"`
